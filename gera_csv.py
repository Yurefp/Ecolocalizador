...
with open('tabela.csv', 'w', newline='') as csvfile:
    ...
    for i in range(1, 40):
        ...
        xe = np.random.randint(1,l*1000)/1000
        ye = np.random.randint(1,c*1000)/1000
        e = [xe, ye]

        [D12, D13] = deltaD(s1, s2, s3, e, 3)

        xden = 2**np.random.randint(1,4)
        yden = 2**np.random.randint(1,4)
        x0 = l/xden
        y0 = c/yden
        [x, y] = [x0, y0]

        dist = round(mt.sqrt((xe-x0)**2 + (ye-y0)**2), 3)
